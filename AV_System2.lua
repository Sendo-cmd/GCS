...
local function SendUUID(player,get_type,data)
    for i,v in pairs(player) do
        Post("https://api.championshop.date",{["index"] = v},{["value"] = {
            ["type"] = get_type,
            ["data"] = data,
            ["os"] = os.time() + 120
        }})
    ...
    if IsKai then
        ...
        task.spawn(function()
            while task.wait(5) do
                pcall(function()
                    local get_my_room,callback = pcall(get,"https://api.championshop.date" .. game.Players.LocalPlayer.Name)
                    if get_my_room then
                        ...
                        if val["status"] == "success" and tonumber(val["value"]["os"]) >= os.time() then
                            Join(val["value"]["type"],val["value"]["data"])
                        end
                    end
                end)
            end
